...
Size_Cell = cell(1, length(Thickness_range));
Contrast = ones(length(Thickness_range));
filtered = Size_Cell;
for k=1:length(Thickness_range)
    ...
    for n=1:size(RectangleLocation, 1)
    ...
    min_sim = min(abs(filtered{k}).^2,[],'all'); %returns the smallest element of the simulation (in terms of intensity). .^2 makes it in terms of intensity.
    Contrast(k)=(1-min_sim);
end
%%
